...
SELFISH_range = np.arange(0,1.01,0.25)
for SELFISH in SELFISH_range:
    ...
    N_ITERATIONS_TRAIN = int(1e4)
    N_LOG_INTERVAL = int(N_ITERATIONS_TRAIN/4)
    ...
    #SELFISH =  0.5 #  np.random.choice(SELFISH_range) #round(np.random.uniform(0, 1), 3) #SELFISH_range[iterator] 
    
    # Chose the local vs global reward weights (only local matters: SELFISH = 1)
    # (only global reward matters: SELFISH = 0)
    
    #SELFISH_range = [0, 0, 0.25, 0.25, 0.33, 0.33, 0.5, 0.5, 0.66, 0.66, 0.75, 0.75, 1, 1] # np.arange(0,1.01,0.1)
    #SELFISH_range = [float(round(i,1)) for i in np.arange(0,1.01,0.25)]
    ...
    ITER_OPS = 10 #len(SELFISH_range)
    
    EXPERIMENT_FOLDER = f'./results/{alg}/{N_AGENTS}-{alg}-{SELFISH}-{MSGS_ACT}-ZssZ'
    ...
    for iterator in range(ITER_OPS):
        ...
        RESNET = False # bool(np.random.choice([True, False]))
        ...
        # MSGS_ACT = True # bool(np.random.choice([True, False]))
        ...
        #SEED = [2, 95, 28, 58, 52, 97, 85,  8, 16, 16, 65,  7, 95, 15, 95, 55, 46]
        ...
        #mean_q_step, mean_steps, mean_ep_rw = pTT.plot_train_test(train = train, test = test, n=int(1e6))
